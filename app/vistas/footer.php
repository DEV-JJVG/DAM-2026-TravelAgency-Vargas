<footer id="footer">
    <div class="footer-legal-bar">
        <nav class="footer-legal-links" aria-label="Navegación legal">
            <a href="condiciones_uso.php" id="link-condiciones-uso">Condiciones de uso</a>
            <span class="footer-legal-sep" aria-hidden="true">|</span>
            <a href="politica_privacidad.php" id="link-politica-privacidad">Política de privacidad</a>
            <span class="footer-legal-sep" aria-hidden="true">|</span>
            <a href="condiciones_transporte.php" id="link-condiciones-transporte">Condiciones de transporte</a>
        </nav>
    </div>

    <div class="footer-credits">
        <p>© <?php echo date('Y'); ?> Agencia de Viajes &mdash; Jorge Juan Vargas Gálvez · 2ºDAM A</p>
    </div>
</footer>